...
class ExLlamaV2ForCausalLM(GenerationMixin):
        ...
        **kwargs,
        ...
        if labels is None:
            if past_key_values is None:
                past_key_values = ExLlamaV2Cache(
                    self.model, input_ids.shape[0], cache_size
                )
                self.model.forward(
                    input_ids[..., :-1],
                    past_key_values,
            ...
            logits = self.model.forward(
                input_ids, past_key_values, loras=loras, input_mask=attention_mask
            )
    ...
    def _reorder_cache(past_key_values: ExLlamaV2Cache, beam_idx):
        ...
        for i in range(len(past_key_values.key_states)):
            past_key_values.key_states[i] = past_key_values.key_states[i].index_select(
                0, beam_idx.to(past_key_values.key_states[i].device)
            )
            past_key_values.value_states[i] = past_key_values.value_states[
                i
            ].index_select(0, beam_idx.to(past_key_values.value_states[i].device))
